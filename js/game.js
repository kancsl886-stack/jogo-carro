...
const Game = {
  ...
  drawEntity(ctx, e) {
    ...
    if (e.kind === "coin") {
      ...
      if (!p.visible) return;
      drawCrateAsset(ctx, p.x, p.y, this.spriteSize(p, 1.05), 3 + ((e.z | 0) % 3));
    } else if (e.kind === "cone") {
      const p = this.project(x, 0, e.z);
      if (!p.visible) return;
      drawConeAsset(ctx, p.x, p.y, this.spriteSize(p, 0.9));
    } else if (e.kind === "tires") {
      const p = this.project(x, 0, e.z);
      if (!p.visible) return;
      drawTireStackAsset(ctx, p.x, p.y, this.spriteSize(p, 0.95));
    } else if (e.kind === "oil") {
      const p = this.project(x, 0.02, e.z);
      if (!p.visible) return;
      drawOilAsset(ctx, p.x, p.y, this.spriteSize(p, 1.15));
    } else if (e.kind === "container") {
      const p = this.project(x, 0, e.z);
      if (!p.visible) return;
      drawContainerAsset(ctx, p.x, p.y, this.spriteSize(p, 1.45));
